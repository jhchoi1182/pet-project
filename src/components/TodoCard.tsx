...
export default function TodoCard({ todo: { id, title, contents, isDone } }: TodoCardProps) {
  ...
  return (
    <li className="flex flex-col w-72 h-60 border-[3px] border-teal-500 rounded-xl pt-3 px-6 pb-6 hover:shadow-lg">
      ...
      <div className="flex gap-7 mt-auto pt-2">
        <button className={`${BUTTON_STYLE} border-red-500`} onClick={() => todoChangeHandler(id, "delete")}>
          삭제하기
        </button>
        <button className={`${BUTTON_STYLE} border-green-600`} onClick={() => todoChangeHandler(id, "update", isDone)}>
          {isDone ? "취소" : "완료"}
        </button>
      </div>
    </li>
