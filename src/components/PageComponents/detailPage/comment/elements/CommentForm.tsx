...
export default function CommentForm({ todoId }: { todoId: number }) {
  ...
  return (
    <form className="flex justify-center gap-10 w-full my-14" onSubmit={(event) => handlePost({ event, mutate, comment, setComment })}>
      <input className="w-[60%] py-2 px-3 border border-teal-500 rounded-lg" name="comment" value={comment} onChange={(event) => handleInputChange(event, setComment)} />
      <Button>댓글쓰기</Button>
    </form>
  );
}
